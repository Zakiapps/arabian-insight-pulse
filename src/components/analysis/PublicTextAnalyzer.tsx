  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (saveError) {
        ...
        // Don't show error to user for saving issues in public interface
      }
        ...
        <CardContent className="space-y-6">
          ...
          {error && (
            <div className="p-4 bg-red-50 border border-red-200 rounded-lg">
              <p className="text-red-700 font-medium text-center">خطأ: {error}</p>
            </div>
          )}
              ...
              <div className="grid gap-4">
                ...
                <div className="flex items-center justify-between p-3 bg-white rounded-lg shadow-sm">
                  ...
                  <div className="text-sm space-y-1">
                    <div>إيجابي: {(result.positive_prob * 100).toFixed(1)}%</div>
                    <div>سلبي: {(result.negative_prob * 100).toFixed(1)}%</div>
                  </div>
                </div>
